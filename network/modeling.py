...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone, mc_dropout):
    ...
    if name=='deeplabv3plus':
        ...
        classifier = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
        if mc_dropout == True:
            new_drop = torch.nn.Dropout(p=0.5, inplace=False)
            classifier.aspp.project[3] = new_drop
    elif name=='deeplabv3':
...
def _load_model(arch_type, backbone, num_classes, output_stride, pretrained_backbone, mc_dropout):

    if backbone=='mobilenetv2':
        model = _segm_mobilenet(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
    elif backbone.startswith('resnet'):
        model = _segm_resnet(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone, mc_dropout=mc_dropout)
...
def deeplabv3plus_resnet101(num_classes=21, output_stride=8, pretrained_backbone=True, mc_dropout = False):
    """Constructs a DeepLabV3+ model with a ResNet-101 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus', 'resnet101', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone, mc_dropout=mc_dropout)
